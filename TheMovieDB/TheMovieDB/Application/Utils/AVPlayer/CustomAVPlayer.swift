...
@objc open class CustomAVPlayer: NSObject, Player, ProvidesView {
    public struct Constants {
        public static let TimeUpdateInterval: TimeInterval = 0.5
    }
    ...
    public private(set) var state: PlayerState = .unknown {
        didSet {
...
//        self.player.currentItem?.audioTimePitchAlgorithm = .timeDomain
    ...
    open func addBoundaryTimeObserver(forTimes times: [NSValue], queue: DispatchQueue?, using block: @escaping () -> Void) {
        self.playerBoundaryTimeObserver = self.player.addBoundaryTimeObserver(forTimes: times, queue: queue, using: block)
    }
    
    open func removeTimeObserver() {
        self.removePlayerBoundaryTimeObserver()
    }
    ...
    private func addPlayerObservers() {
        ...
        self.playerTimeObserver = self.player.addPeriodicTimeObserver(forInterval: interval, queue: DispatchQueue.main, using: { [weak self] (cmTime) in
            ...
            if let strongSelf = self, let time = cmTime.timeInterval, !strongSelf.isSeekInProgress {
    ...
    private func removePlayerObservers() {
        ...
        removePlayerBoundaryTimeObserver()
    ...
    private func removePlayerBoundaryTimeObserver() {
        if let playerBoundaryTimeObserver = self.playerBoundaryTimeObserver {
            self.player.removeTimeObserver(playerBoundaryTimeObserver)
    ...
    private func playerRateDidChange(rate: Float) {
        if Int(self.time) >= Int(self.duration) && self.duration > 0 {
            self.state = .ended
        } else {
            self.state = self.playing ? .playing : .paused
        }
...
//        let state: PlayerState = playbackLikelyToKeepUp ? self.state : .buffering
//
//        self.state = state
    }
...
extension CustomAVPlayer: AirPlayCapable
{
...
#if os(iOS)
extension CustomAVPlayer: PictureInPictureCapable {
    @available(iOS 9.0, *)
...
extension CustomAVPlayer: VolumeCapable
{
...
extension CustomAVPlayer: FillModeCapable {
...
extension CustomAVPlayer: TextTrackCapable {
